    ...
    <div className="space-y-6">
        ...
        <div className="space-y-2">
          {CATEGORIES.map(cat => (
            <label key={cat} className="flex items-center gap-2 cursor-pointer group">
              <input type="radio" name="category" value={cat} checked={filters.category === cat} onChange={() => updateFilter('category', cat === filters.category ? '' : cat)}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input type="number" placeholder="Min" value={filters.min_price} onChange={e => updateFilter('min_price', e.target.value)} className="input-dark text-sm" />
          <input type="number" placeholder="Max" value={filters.max_price} onChange={e => updateFilter('max_price', e.target.value)} className="input-dark text-sm" />
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <aside className="hidden md:block w-56 flex-shrink-0">
            <div className="bg-[#111] rounded-xl border border-[#1A1A1A] p-5 sticky top-24">
              <h2 className="font-semibold mb-4">Filters {activeFiltersCount > 0 && <span className="bg-gold text-black text-xs px-2 py-0.5 rounded-full ml-1">{activeFiltersCount}</span>}</h2>
              <FilterPanel />
            </div>
          </aside>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 mb-6 flex-wrap">
              <button onClick={() => setFilterOpen(true)} className="md:hidden flex items-center gap-2 bg-[#111] border border-[#333] px-4 py-2 rounded-lg text-sm hover:border-gold transition-colors">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                <div className="relative">
                  <select value={filters.sort} onChange={e => updateFilter('sort', e.target.value)} className="bg-[#111] border border-[#333] text-sm px-4 py-2 rounded-lg text-white appearance-none pr-8 focus:outline-none focus:border-gold cursor-pointer">
